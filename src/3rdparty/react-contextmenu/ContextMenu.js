...
export default class ContextMenu extends AbstractMenu {
    ...
    handleShow = (e) => {
        ...
        let { x, y } = e.detail.position;

        switch (e.detail.rotateMode) {
          case 0:
            break;
          case 1:
            {
        ...
        this.setState({ isVisible: true, x, y, rotateMode: e.detail.rotateMode });
    ...
    getMenuPosition = (x = 0, y = 0) => {
        ...
        const { innerWidth, innerHeight } = this.state.rotateMode & 1 
           ? { innerWidth: window.innerHeight, innerHeight: window.innerWidth }
           : window;
        let rect = this.menu.getBoundingClientRect();
        if (this.state.rotateMode & 1) {
          rect = {
            width: rect.height,
            hegiht: rect.width,
          };
